...
class UploadsController extends AjaxMultiUploadAppController {
	...
	public function upload($dir=null) {
		...
		if ($dir === null) {
			$this->set("result", "{\"error\":\"Carregar controlador foi passado um valor nulo.\"}");
			return;
		}
	...
	public function delete($file = null) {
		if(is_null($file)) {
			$this->Session->setFlash(__('Parâmetro o arquivo estiver ausente'));
			$this->redirect($this->referer());
		}
		$file = base64_decode($file);
		if(file_exists($file)) {
			if(unlink($file)) {
				$this->Session->setFlash(__('Arquivo apagado!'));				
			} else {
				$this->Session->setFlash(__('Não é possível excluir arquivo'));					
			}
		} else {
			$this->Session->setFlash(__('O arquivo não existe!'));					
		}
