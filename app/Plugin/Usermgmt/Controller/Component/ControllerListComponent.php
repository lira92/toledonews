...
class ControllerListComponent extends Component {
	...
	public function getControllers() {
		$controllerClasses = App::objects('Controller');
		$contador = 0;
		//limpa controladoras q o usuario não precisa ver
		...
		$controllerClasses[-2]="Selecionar Controller";
		$controllerClasses[-1]="Todos";
		$plugins = App::objects('plugins');
		foreach ($plugins as $p) {
			$pluginControllerClasses = App::objects($p.'.Controller');
			//limpa controladoras q o usuario não precisa ver
			foreach ($pluginControllerClasses as $controller) {
				if($controller != "AjaxMultiUploadAppController" &&  $controller != "UploadsController" &&  $controller != "DebugKitAppController" &&  $controller != "ToolbarAccessController" &&  $controller != "UserMgmtAppController"){	
